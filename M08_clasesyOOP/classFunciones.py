class Funciones:
    def __init__(self, lista_fun):
        if type(lista_fun)!=list:
            self.lista_fun=[]
            raise ValueError('Solo se debe colocar elementos del tipo "list".')
        else:
        ...
        for i in self.lista_fun:
            if type(i)==int:
                if self.sera_primo(i) == True:
                    lis_prim.append(True)
                else:
                    lis_prim.append(False)
            else:
                raise TypeError('La lista debe contener elementos del tipo "int".')
        return lis_prim
